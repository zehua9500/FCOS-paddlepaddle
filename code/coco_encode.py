...
class CocoEncode():
    def __init__(self):
        self.divide = [32, 64, 128, 256]
        self.stride = [8, 16, 32, 64, 128]
        self.size2layer = [[80, 80], [40, 40], [20, 20], [10, 10], [5, 5]]
        self.layerNum = 5
        self.clsNum = 90

    def gt_process(self, gt):
        """
        fcos按bbox的大小 分别分给不同的fpn层
        :param gt:
        :return:
        """
        if gt is None:
            return [np.zeros((0, 5))] * 5
        meanedge = np.sqrt((gt[:, 2] - gt[:, 0]) * (gt[:, 3] - gt[:, 1]))
        gt1 = gt[meanedge <= self.divide[0]]
        gt2 = gt[(meanedge <= self.divide[1]) * (meanedge >= self.divide[0])]
        gt3 = gt[(meanedge <= self.divide[2]) * (meanedge >= self.divide[1])]
        gt4 = gt[(meanedge <= self.divide[3]) * (meanedge >= self.divide[2])]
